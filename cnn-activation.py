...
class CNNclass(torch.nn.Module):
    def __init__(self, nwords, emb_size, num_filters, window_size, ntags, weight_matrix):
        ...
        #     torch.nn.Conv2d(emb_size, 128, kernel_size=window_size, stride=1, padding=0, dilation=1, groups=1, bias=True),
        #     torch.nn.Dropout(0.2),
        #     torch.nn.ReLU())
        # self.layer2 = torch.nn.Sequential(
        #     torch.nn.Conv2d(128, num_filters, kernel_size=window_size, stride=1, padding=0, dilation=1, groups=1, bias=True),
        #     torch.nn.Dropout(0.2),
        #     torch.nn.ReLU())
...
# para tuning
optimizer = torch.optim.Adam(model.parameters(), lr=1e-5, eps=1e-8, weight_decay=1e-4)
